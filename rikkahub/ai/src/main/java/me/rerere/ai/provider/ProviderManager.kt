...
class ProviderManager(client: OkHttpClient) {
    ...
    fun <T : ProviderSetting> getProviderByType(setting: T): Provider<T> {
        ...
        return when (setting) {
            ...
            // FIX: Add an else branch to make the 'when' expression exhaustive.
            else -> throw IllegalArgumentException("Unsupported provider setting type: ${setting::class.simpleName}")
        } as Provider<T>
    }
}
