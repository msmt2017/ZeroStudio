...
class GradleHandler {
    ...
    suspend fun handleExecuteCommand(args: Map<String, String>, sendChannel: SendChannel<McpResponse>) {
        ...
        if (command.isEmpty()) {
            sendChannel.send(McpResponse(UUID.randomUUID().toString(), "gradle.execute.error", "No command specified", ""))
            sendChannel.close()
            return
        }
        ...
        try {
            ...
            if (buildService == null) {
                sendChannel.send(McpResponse(UUID.randomUUID().toString(), "gradle.execute.error", "BuildService unavailable", ""))
                sendChannel.close()
                return
            }

            sendChannel.send(McpResponse(UUID.randomUUID().toString(), "gradle.execute.started", "Executing: $command", ""))
            // 简单处理：将命令按空格分割为任务数组
            val tasks = command.split("\\s+")
                .filter { it.isNotEmpty() }
            buildService.executeTasks(*tasks.toTypedArray())
            sendChannel.send(McpResponse(UUID.randomUUID().toString(), "gradle.execute.complete", "{\"isSuccessful\":true}", ""))
        } catch (e: Exception) {
            val errMsg = e.localizedMessage ?: "Failed to execute command"
            sendChannel.send(McpResponse(UUID.randomUUID().toString(), "gradle.execute.error", errMsg, ""))
        } finally {
            sendChannel.close()
        }
    }
