...
// Define the escapeJavaString function at the top
fun escapeJavaString(s: String): String {
    return s.replace("\\", "\\\\")
            .replace("\"", "\\\"")
            .replace("\n", "\\n")
            .replace("\r", "\\r")
...
// Task definition using tasks.register
    ...
    doLast {
        buildInfoIn.replaceContents(
            ...
            candidates = arrayOf(
                ...
                "CI_GIT_BRANCH" to escapeJavaString(CI.branchName),
                "CI_COMMIT_HASH" to escapeJavaString(CI.commitHash),
